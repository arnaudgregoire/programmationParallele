/*
 * Programmation Parallèle - Fevrier 2018
 * ENSG 
 * by Ahmad AUDI 
...
int main(int argc, char *argv[]) {
  ...
  if(rank == MASTER){
    ...
    int nb_para = h/nb_proc;
    int rest = h%nb_proc;
    printf("Rest : %d\n", rest);
    printf("Number of parallelizations : %d\n", nb_para);

  }

  MPI_Bcast(&h, sizeof(unsigned char), MPI_INT, MASTER, MPI_COMM_WORLD);
  MPI_Bcast(&w, sizeof(unsigned char), MPI_INT, MASTER, MPI_COMM_WORLD);
    
  /* debut du chronometrage */
  debut = my_gettimeofday(); 

  if(rank == MASTER || rank == nb_proc-1){
    h_loc++;
  }else{
    h_loc += 2;
  }

  h_loc = h_loc+h/nb_proc;
  // printf("%d H_LOC : %d\n", rank, h_loc);

  pima = ima = (unsigned char*)malloc(w*h_loc*sizeof(unsigned char));

  if(rank >= 1 && rank <= nb_proc-1){
    w_scatter = w;
  }else{
    w_scatter = 0;
  }

  MPI_Scatter(r.data, w*(h/nb_proc), MPI_UNSIGNED_CHAR, pima + w_scatter, w*(h/nb_proc), MPI_UNSIGNED_CHAR, MASTER, MPI_COMM_WORLD);

  /* La convolution a proprement parler */
  for(i=0 ; i < nbiter ; i++){
    if(rank > 0){
      MPI_Send(ima+w, w, MPI_UNSIGNED_CHAR, rank - 1, 0, MPI_COMM_WORLD);
      // printf("%d SENDED TO %d\n", rank, rank - 1);
      MPI_Recv(ima, w, MPI_UNSIGNED_CHAR, rank - 1, 0, MPI_COMM_WORLD, &status);
      // printf("%d RECEIVED FROM %d\n", rank, status.MPI_SOURCE);
    }
    if(rank < nb_proc-1){
      MPI_Recv(ima+(w*(h_loc-1)), w, MPI_UNSIGNED_CHAR, rank + 1, 0, MPI_COMM_WORLD, &status);
      // printf("%d RECEIVED FROM %d\n", rank, status.MPI_SOURCE);
      MPI_Send(ima+(w*(h_loc-2)), w, MPI_UNSIGNED_CHAR, rank + 1, 0, MPI_COMM_WORLD);
      // printf("%d SENDED TO %d\n", rank, rank + 1);
    }
    convolution(filtre, ima, h_loc, w);
  } /* for i */
  ...
  // printf("GATHERING\n");
  MPI_Gather(ima+w_scatter, w*(h/nb_proc), MPI_UNSIGNED_CHAR, r.data, w*(h/nb_proc), MPI_UNSIGNED_CHAR, MASTER, MPI_COMM_WORLD);
  // printf("GATHERED\n");
